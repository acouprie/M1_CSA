...
void * producer (void *arg) {
  ...
  for (i = 0; i < NB_TIMES_PROD; i++) {
    ...
    if(resCritiques.posR == resCritiques.posW - 1) {
      sem_post(&typeMsg[theMessage.typeOfMessage]);
    }
...
void * consumer (void *arg) {
  ...
  for (i = 0; i < NB_TIMES_CONS; i++) {
    ...
    sem_wait(&typeMsg[param->typeOfMessage]);
    ...
    if(resCritiques.buffer[resCritiques.posR].typeOfMessage != -1) {
      sem_post(&typeMsg[resCritiques.buffer[resCritiques.posR].typeOfMessage]);
    }
...
//	srand((int)pthread_self());
  ...
  for (i = 0; i <  nbThds; i++) {
    if (i < nbProd) {
      ...
      paramThds[i].typeOfMessage = (i - nbProd) % NB_TYPES;
