...
class PointTest {
	...
	public void testTranslate() {   
		...
		float oldOrdonne = this.p.getOrdinate();   
		this.p.translate(1.1F, 2.2F);   
		assertEquals(oldAbscisse + 1.1F, this.p.getAbscisse());   
		assertEquals(oldOrdonn�e + 2.2F, this.p.getOrdinate());  
	}    

	@Test  
	public void testToScale() {   
		float oldAbscisse = this.p.getAbscisse();   
		float oldOrdonn�e = this.p.getOrdinate();   
		this.p.toScale(1.5F);   
		assertEquals(oldAbscisse * 1.5F, this.p.getAbscisse());   
		assertEquals(oldOrdonn�e * 1.5F, this.p.getOrdinate());  
	}  
